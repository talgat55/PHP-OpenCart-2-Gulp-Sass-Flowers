...
class ModelCatalogShopRating extends Model {
	...
	public function install() {
		...
		$query_name = $this->db->query("SELECT query FROM `" . DB_PREFIX . "url_alias` WHERE `keyword` LIKE 'shoprating' LIMIT 1");
		if ($query_name->num_rows == 0) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'information/shop_rating', keyword = 'shoprating'");
		}
