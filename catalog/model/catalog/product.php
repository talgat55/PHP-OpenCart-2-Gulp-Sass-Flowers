...
class ModelCatalogProduct extends Model
{
    ...
    public function minPriceProducts()
    {
        $sql = 'SELECT MIN(' . DB_PREFIX . 'product.price) FROM ' . DB_PREFIX . 'product';
    ...
    public function maxPriceProducts()
    {
        $sql = 'SELECT MAX(' . DB_PREFIX . 'product.price) FROM ' . DB_PREFIX . 'product';
    ...
    public function getProductsByAttributeAndPrice($data = array(), $price = null)
    {
...
//        $sql = "SELECT * FROM ". DB_PREFIX . "product_attribute  pr   WHERE  pr.attribute_id IN (16)   LEFT JOIN ". DB_PREFIX . "product pa  ON  pr.attribute_id=pa.product_id";


        $sql = "SELECT * FROM ". DB_PREFIX . "product pa INNER JOIN   ". DB_PREFIX . "product_attribute pr  WHERE  pr.attribute_id IN (".$data.") ";
//        if (empty($ext)) {
...
//            if (!empty($ext['categories'])) {
//
//                $sql .= " AND p2c.category_id  IN (" . $ext['categories'] . ")";
//
//            }
//
//
...
//        }
    ...
    public function getMinPriceProducts($data = array())
    {
        $query = $this->db->query("
    ...
    public function getProducts($data = array(), $ext = null)
    {
        $sql = "SELECT p.product_id, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special";
