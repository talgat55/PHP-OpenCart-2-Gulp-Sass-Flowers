...
//		}
        ...
        if ($this->customer->isLogged()) {
            ...
            $array_wish = array_unique($redyArr);
            array_shift($array_wish);

            $results = [];
            foreach ( $array_wish as   $key =>$value ){
    ...
    public function add()
    {
        ...
        if ($product_info) {
            if ($this->customer->isLogged()) {
                ...
                setcookie("wishlistGuest", $cookies_wishlist . ',' . $this->request->post['product_id']);
...
//				$this->session->data['wishlist'][] = $this->request->post['product_id'];
                ...
                $redyArr = explode(',', $cookies_wishlist);
                $redyArrUniq = array_unique($redyArr);
                $json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'product_id=' . (int)$this->request->post['product_id']), $product_info['name'], $this->url->link('account/wishlist'));

                $json['total'] = isset($redyArrUniq) ? count($redyArrUniq) : 0;
            }
        }
